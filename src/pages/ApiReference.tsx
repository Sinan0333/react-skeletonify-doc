  ...
  const components = [
    {
      ...
      description: 'Smart wrapper component that automatically creates skeleton placeholders for your components',
      props: [
        { name: 'loading', type: 'boolean', default: 'false', description: 'Controls whether to show skeleton or actual content' },
        { name: 'children', type: 'ReactNode', default: '-', description: 'The component to wrap with skeleton functionality' },
        { name: 'config', type: 'SkeletonConfig', default: '{}', description: 'Optional configuration to override global settings' },
      ]
    },
    {
      name: 'SkeletonProvider',
      description: 'Provider component for setting global skeleton configuration across your entire app',
      props: [
        { name: 'config', type: 'SkeletonConfig', default: '{}', description: 'Global configuration object' },
        { name: 'children', type: 'ReactNode', default: '-', description: 'Your app components' },
      ]
    }
  ];
        ...
        <div className="space-y-12">
          ...
          <section>
            <div className="flex items-center space-x-3 mb-8">
              ...
              <h2 className="text-3xl font-semibold text-white">Configuration Options</h2>
            </div>

            <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
              <div className="p-6 border-b border-slate-700">
                <h3 className="text-xl font-semibold text-white mb-2">SkeletonConfig</h3>
                <p className="text-slate-300">Configuration object accepted by both SkeletonProvider and SkeletonWrapper</p>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-slate-700">
                    <tr>
                      <th className="text-left p-4 text-slate-200 font-medium">Option</th>
                      <th className="text-left p-4 text-slate-200 font-medium">Type</th>
                      <th className="text-left p-4 text-slate-200 font-medium">Default</th>
                      <th className="text-left p-4 text-slate-200 font-medium">Description</th>
                    </tr>
                  ...
                  <tbody>
                    {configOptions.map((option, index) => (
                      <tr key={index} className="border-t border-slate-700">
                        <td className="p-4 text-slate-300 font-mono text-sm">{option.name}</td>
                        <td className="p-4 text-slate-400 font-mono text-sm">{option.type}</td>
                        <td className="p-4 text-slate-400 font-mono text-sm">{option.default}</td>
                        <td className="p-4 text-slate-300 text-sm">{option.description}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <section>
            <div className="flex items-center space-x-3 mb-8">
              ...
              <h2 className="text-3xl font-semibold text-white">Usage Examples</h2>
            </div>
    ...
    <SkeletonWrapper loading={loading}>
      <div>
        ...
        <p>Content goes here</p>
              ...
              <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
                <h3 className="text-xl font-semibold text-white mb-4">With Global Configuration</h3>
                <div className="bg-gray-900 rounded-lg p-4 border border-slate-600 overflow-x-auto">
                  <pre className="text-slate-300 font-mono text-sm">
